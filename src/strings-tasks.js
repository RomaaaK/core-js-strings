...
function isPalindrome(str) {
  const strPrepare = str
    .replaceAll(' ', '')
    .replaceAll(',', '')
    .replaceAll('!', '')
    .replaceAll('?', '')
    .toLowerCase();
  const first = strPrepare.substring(0, Math.floor(strPrepare.length / 2));
  const second = strPrepare.substring(Math.round(strPrepare.length / 2));
...
function reverseWords(str) {
  ...
  arrStr = arrStr.map((i) => reverseString(i));
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((letter) => {
    const position = alphabetArr.indexOf(letter.toLowerCase());
    if (position !== -1) {
      let newPosition = position + 13;

      if (newPosition >= alphabetArr.length) {
        newPosition -= alphabetArr.length;
      }

      if (letter === letter.toUpperCase()) {
        return alphabetArr[newPosition].toUpperCase();
      }

      return alphabetArr[newPosition];
    }
